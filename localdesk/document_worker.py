...
def ocr_words(image_path: Path, language='eng') -> list[dict]:
    ...
    try:
        result = subprocess.run([binary, str(image_path), 'stdout', '-l', language, '--psm', '6', 'tsv'],
                                capture_output=True, text=True, encoding='utf-8', errors='replace',
                                timeout=30, check=False)
    except subprocess.TimeoutExpired as exc:
        raise InputError('OCR reached its 30 second page limit.') from exc
    if result.returncode:
        raise InputError('Tesseract could not read this page. Check the installed language data.')
    words = []
...
def render_page(path: Path, number: int, scale=2):
    ...
    try:
        ...
        try:
            ...
            if not all(math.isfinite(v) and v > 0 for v in (width, height)) or width*height*scale*scale > MAX_PIXELS:
    ...
    if path.suffix.lower() == '.pdf':
        ...
        with pdfplumber.open(str(path)) as pdf:
            if len(pdf.pages) > max_pages:
                raise InputError(f'This PDF has more than {max_pages} pages. Split the document first.')
            for index, page in enumerate(pdf.pages):
                native = page.extract_words()[:MAX_WORDS+1]
                if len(native) > MAX_WORDS:
                    raise InputError('This page has too many text objects.')
                words = [{'text': w['text'], 'bbox': [w['x0'], w['top'], w['x1'], w['bottom']],
                          'line': round(w['top']/3)} for w in native]
                method = 'PDF text'
                if ocr:
                    ...
                    for word in recognized:
                        ...
                        if not any(w['bbox'][0] <= center_x <= w['bbox'][2] and
                                   w['bbox'][1]-2 <= center_y <= w['bbox'][3]+2 for w in words[:len(native)]):
                            words.append(word)
                    if len(words) > MAX_WORDS:
                        raise InputError('This page has too many combined native and OCR words.')
                    ...
                    warnings.append(f'Page {index+1}: no selectable text. Enable OCR for this scan.')
                pages.append({'page': index+1, 'width': float(page.width), 'height': float(page.height),
                              'words': words, 'method': method})
                page.close()
    else:
        ...
        with Image.open(path) as original:
            ...
            pages.append({'page': 1, 'width': width, 'height': height, 'words': words,
    ...
    for page in pages:
        ...
        for word in page['words']:
            ...
            if position > MAX_TEXT_CHARS:
                raise InputError('This document exceeds 250,000 extracted characters. Split the input.')
        parts.append('\n')
        position += 1
    text = ''.join(parts)
    return {'text': text, 'pages': pages, 'warnings': list(dict.fromkeys(warnings)),
            'truncated': False, 'method': ' + '.join(sorted({p['method'] for p in pages}))}


def tables(path: Path, *, strategy='lines', language='eng', column_edges=None, **unused) -> dict:
    if path.suffix.lower() != '.pdf':
    ...
    if strategy in {'lines', 'text'}:
        ...
        with pdfplumber.open(path) as pdf:
            if len(pdf.pages) > MAX_PAGES:
                raise InputError('Split PDFs with more than 50 pages before extracting tables.')
            for page in pdf.pages:
                ...
                for index, found in enumerate(page.find_tables(settings)):
                    ...
                    output.append({'page': page.page_number, 'table': index+1,
                                   'bbox': list(found.bbox), 'rows': [[str(v or '') for v in row] for row in rows],
        ...
        for page in data['pages']:
            ...
            if edges and (any(isinstance(x, bool) or not isinstance(x, (int,float)) or not math.isfinite(x) or not 0<x<page['width'] for x in edges) or edges != sorted(set(edges))):
                raise InputError('Column boundaries must be increasing page x coordinates.')
            groups = {}
            for word in page['words']:
                key = str(word['line'])
            ...
            for group in groups.values():
                ...
                if edges:
                    ...
                    row=[]; right=None
                    for w in group:
                        if right is None or w['bbox'][0]-right>max(14,page['width']/30):
                            row.append(w['text'])
                        else:
                            row[-1]+=' '+w['text']
                        right=w['bbox'][2]
                rows.append(row)
            if rows:
                output.append({'page':page['page'],'table':1,'bbox':[0,0,page['width'],page['height']],
                               'rows':rows,'method':'OCR/word-position table candidates'})
    cells=sum(len(row) for t in output for row in t['rows'])
    if cells>20000:
        raise InputError('The combined tables exceed 20,000 cells.')
    return {'tables':output,'warnings':['Check cell boundaries, merged cells, and OCR values before using this export.'],
            'strategy':strategy,'table_count':len(output)}
...
def main():
    try:
        ...
        message=str(exc)[:350] if isinstance(exc,(InputError,ImportError)) else f'The parser rejected this document ({type(exc).__name__}).'
