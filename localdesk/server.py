...
def make_server(app, config: dict, port: int = 0):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _host_ok(self) -> bool:
            ...
            return len(self.headers.get_all('Host', [])) == 1 and self.headers.get('Host', '') in allowed

        def _origin_ok(self) -> bool:
            origin = self.headers.get('Origin')
            return origin is None or origin in {f'http://127.0.0.1:{self.server.server_port}',
                                               f'http://localhost:{self.server.server_port}'}

        def _authorized(self) -> bool:
            supplied = self.headers.get('X-Local-Token', '')
            return self._host_ok() and self._origin_ok() and hmac.compare_digest(supplied, token)

        def _send(self, code: int, raw: bytes, kind: str = 'application/json; charset=utf-8',
        ...
        def do_GET(self):
            if not self._host_ok() or not self._origin_ok():
                return self._json(403, {'error': 'Only this local app origin is allowed.'})
            request = urlsplit(self.path)
            if request.path == '/healthz':
                return self._json(200, {'status': 'ok', 'name': config['name']})
            if request.path.startswith('/api/'):
                if not self._authorized():
                    return self._json(403, {'error': 'The local session token is missing or invalid.'})
                ...
                try:
                    ...
                    return self._json(500, {'error': 'The operation failed. Check the app input and try again.'})
            name = request.path.lstrip('/') or 'index.html'
            path = (static / name).resolve()
            if not within(path, static) or not path.is_file() or path.suffix not in {'.html', '.css', '.js', '.svg', '.png', '.gif', '.ico'}:
                return self._json(404, {'error': 'This page does not exist.'})
            raw = path.read_bytes()
            if name == 'index.html':
                raw = raw.replace(b'@@TOKEN@@', token.encode('ascii'))
            return self._send(200, raw, mimetypes.guess_type(str(path))[0] or 'application/octet-stream')

        def do_POST(self):
            if not self._authorized():
                return self._json(403, {'error': 'This request is not authorized by the local app.'})
            if len(self.headers.get_all('Content-Length', [])) != 1:
                return self._json(400, {'error': 'Use exactly one Content-Length header.'})
            ...
            try:
                ...
                if not 0 < length <= MAX_REQUEST:
                    return self._json(413, {'error': 'The request exceeds the local size limit.'})
...
def serve(app, config: dict, port: int, open_browser: bool):
    ...
    print('Press Ctrl+C to stop. No files are scanned until you start an operation.', flush=True)
    if open_browser:
