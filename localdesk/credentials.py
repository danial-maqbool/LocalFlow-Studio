...
def account(data: Path) -> tuple[str,str]:
    return 'org.localfirst.vault',hashlib.sha256(str(data.resolve()).encode()).hexdigest()
...
def secure_backend():
    ...
    for backend in get_all_keyring():
        try:
            ...
            continue
    if not choices:
        raise InputError('No supported secure OS credential store is available. Use an interactive vault password instead.')
...
def load_password(data: Path) -> str:
    ...
    if not result:
        raise InputError('No vault password is saved. Run service --install from an interactive terminal first.')
    return result
...
def save_password(data: Path, password: str):
    ...
    try:
        ...
        raise InputError('The OS credential store did not save the vault password.') from exc
...
def delete_password(data: Path):
    ...
    try:
        ...
        raise InputError('The service was removed, but its saved credential could not be deleted. Remove that credential in the OS credential manager.') from exc
