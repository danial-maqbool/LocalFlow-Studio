...
class JobContext:
    ...
    def check(self) -> None:
        if self.signal.is_set():
            raise Cancelled('The job was cancelled. Check any outputs and desktop actions that already completed.')

    def progress(self, value: int, message: str = '') -> None:
        self.check()
        self.manager.store.execute('UPDATE jobs SET progress=?,message=? WHERE id=?',
...
class Jobs:
    def __init__(self, store: Store):
        ...
        self.store.execute("UPDATE jobs SET status='interrupted',error=? "
                           "WHERE status IN ('queued','running')",
                           ('The previous app session ended before this job finished.',))

    def submit(self, kind: str, work) -> str:
        with self.lock:
            active = self.store.one("SELECT COUNT(*) AS n FROM jobs WHERE status IN ('queued','running')")['n']
            if active >= 8:
    ...
    def _run(self, job_id: str, signal: threading.Event, work) -> None:
        ...
        try:
            ...
            result = work(context)
            context.check()
            self.store.execute("UPDATE jobs SET status='done',finished=?,progress=100,result=? WHERE id=?",
                               (utcnow(), json.dumps(result, ensure_ascii=False), job_id))
        except Cancelled as exc:
            self.store.execute("UPDATE jobs SET status='cancelled',finished=?,error=? WHERE id=?",
                               (utcnow(), str(exc), job_id))
        except Exception as exc:
            self.store.execute("UPDATE jobs SET status='failed',finished=?,error=? WHERE id=?",
    ...
    def recent(self) -> list[dict]:
        return self.store.rows('SELECT id,kind,status,created,finished,progress,message,error FROM jobs '
