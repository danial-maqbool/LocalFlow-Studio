...
class ProcessLock:
    ...
    def __init__(self, path: Path):
        ...
        try:
            ...
            os.close(self.fd)
            self.fd = None
            raise InputError('This data folder is already open. Use the running app or another data folder.') from exc
...
class Vault:
    def __init__(self, path: Path, password: str | None, *, ephemeral: bool = False):
        ...
        try:
            ...
            raw = b'' if ephemeral or not path.exists() else path.read_bytes() if path.stat().st_size <= MAX_VAULT_BYTES else None
            if not ephemeral and path.exists() and raw == b'':
                raise InputError('The existing vault is empty or truncated. Restore a verified backup.')
            if raw is None:
                raise InputError('The encrypted database exceeds 256 MiB.')
            if raw and not raw.startswith(MAGIC):
                raise InputError('This database is not an encrypted v2 vault. Use the documented migration command.')
            ...
            if raw:
                offset = len(self.aad)
                try:
                    snapshot = self.cipher.decrypt(raw[offset:offset+12], raw[offset+12:], self.aad)
                except InvalidTag as exc:
                    raise InputError('The passphrase is wrong or the encrypted database is damaged.') from exc
                self.db.deserialize(snapshot)
                self.last_snapshot = snapshot
                if self.db.execute('PRAGMA quick_check').fetchone()[0] != 'ok':
                    raise InputError('The decrypted database failed its integrity check.')
    ...
    def connection(self):
        with self.lock:
            ...
            try:
                with self.db:
                    yield self.db
                changed = (self.db.total_changes != before_changes or
                           self.db.execute('PRAGMA schema_version').fetchone()[0] != before_schema)
                if changed:
                    snapshot = self.db.serialize()
                    if len(snapshot) > MAX_VAULT_BYTES - 256:
                        raise InputError('The database reached its 256 MiB safety limit. Remove old history.')
    ...
    def backup(self, destination: Path):
        with self.lock:
            ...
            with os.fdopen(os.open(destination, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), 'wb') as f:
                f.write(raw)
                f.flush()
                os.fsync(f.fileno())
...
def migrate_plaintext(source: Path, destination: Path, password: str) -> None:
    ...
    if source.resolve() == destination.resolve() or destination.exists():
        raise InputError('Choose a new destination. The source will not be overwritten.')
    if source.is_symlink() or not source.is_file() or source.stat().st_size > MAX_VAULT_BYTES:
