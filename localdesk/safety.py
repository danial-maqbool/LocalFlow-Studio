...
DEFAULT_EXCLUDES = ('.git', '.venv', 'venv', 'node_modules', '__pycache__',
                    '.local-data', '.localflow', '.env', '.ssh', '.aws', '.config', 'AppData')
...
def integer(value: object, low: int, high: int, label: str = 'Value') -> int:
    ...
    if isinstance(value, bool) or (isinstance(value, float) and not value.is_integer()) or not low <= number <= high:
...
def checked_path(value: str | Path, *, directory: bool = False, max_bytes: int | None = MAX_FILE_BYTES) -> Path:
    ...
    for part in (original, *original.parents):
        ...
        if hasattr(part, 'is_junction') and part.is_junction():
            raise InputError('Directory junctions are not processed. Select the real path.')
    try:
...
def excluded(path: Path, root: Path, patterns: list[str] | tuple[str, ...]) -> bool:
    ...
    return any(fnmatch.fnmatch(part, pat) or fnmatch.fnmatch(relative.as_posix(), pat)
               for part in relative.parts for pat in patterns)
...
def walk_files(root: Path, *, patterns=DEFAULT_EXCLUDES, limit: int = 20_000):
    ...
    for base, dirs, files in os.walk(root, followlinks=False, onerror=onerror):
        ...
        for name in sorted(files):
            ...
            count += 1
            if count > limit:
                raise InputError(f'The folder exceeds the {limit:,} file limit. Select a smaller folder.')
            yield p
...
def archive_name_ok(name: str) -> bool:
    ...
    return not p.is_absolute() and '..' not in p.parts and all(len(s) <= 180 for s in p.parts)
...
def open_zip(raw: bytes) -> zipfile.ZipFile:
    ...
    if any(i.file_size > MAX_FILE_BYTES or
           (i.file_size > 1_000_000 and i.file_size / max(i.compress_size, 1) > 200)
           for i in infos):
        archive.close()
        raise InputError('The archive contains an oversized or highly compressed entry.')
    return archive
