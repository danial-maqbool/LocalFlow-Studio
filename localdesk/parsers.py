...
from .safety import (InputError, MAX_FILE_BYTES, MAX_TEXT_CHARS,
                     checked_path, open_zip, safe_xml)

TEXT_EXTENSIONS = {'.txt', '.md', '.csv', '.tsv', '.json', '.jsonl', '.log', '.py',
                   '.js', '.ts', '.jsx', '.tsx', '.html', '.css', '.xml', '.yaml',
                   '.yml', '.toml', '.ini', '.cfg', '.rst', '.sql', '.sh', '.bat',
                   '.c', '.cpp', '.h', '.java', '.rs', '.go', '.srt', '.vtt'}
...
def office_text(raw: bytes, extension: str) -> str:
    with open_zip(raw) as archive:
        ...
        if extension == '.docx':
            selected = sorted(n for n in names if n == 'word/document.xml' or
            ...
            for name in sorted(n for n in names if re.fullmatch(r'xl/worksheets/sheet\d+\.xml', n)):
                ...
                for row in tree.iter():
                    ...
                    cells = []
                    for cell in row:
                        values = [n.text or '' for n in cell.iter()
                                  if n.tag.rsplit('}', 1)[-1] in {'v', 't'}]
                        value = ''.join(values)
                        if cell.attrib.get('t') == 's' and value.isdigit():
                            idx = int(value)
                            value = shared[idx] if idx < len(shared) else '[invalid shared string]'
                        cells.append(value)
                    value = '\t'.join(cells)
                    chunks.append(value)
                    length += len(value) + 1
...
def extract_bytes(raw: bytes, extension: str) -> dict:
    ...
    if extension in TEXT_EXTENSIONS:
        ...
        return {'text': text[:MAX_TEXT_CHARS], 'method': encoding,
                'truncated': len(text) > MAX_TEXT_CHARS, 'warnings': []}
    if extension in OFFICE_EXTENSIONS:
        text = office_text(raw, extension)
        return {'text': text, 'method': 'Office XML', 'truncated': len(text) >= MAX_TEXT_CHARS,
                'warnings': ['Images, charts, and embedded objects are not converted to text.']}
    raise InputError(f'Text extraction is not available for {extension or "this file type"}.')
...
def extract(path: Path, *, ocr: bool = False) -> dict:
    ...
    if ext in IMAGE_EXTENSIONS:
        return {'text': '', 'method': 'Filename and metadata only', 'truncated': False,
