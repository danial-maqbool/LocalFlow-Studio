...
def validate_actions(actions):
    ...
    for action in actions:
        ...
        kind=action['type'];item={'type':kind}
        if kind in {'move','click'}:
            ...
            if kind=='click':
                ...
                if item['button'] not in {'left','right','middle'}:raise InputError('Choose a valid mouse button.')
            ...
            if not isinstance(text,str) or len(text)>4000 or not text.isascii():
                raise InputError('Keyboard text must be ASCII and no longer than 4,000 characters.')
            item['text']=text
        elif kind in {'press','hotkey'}:
            keys=action.get('keys',[action.get('key','enter')])
            if isinstance(keys,str):keys=[keys]
            if not isinstance(keys,list) or not 1<=len(keys)<=5 or any(not isinstance(k,str) or len(k)>20 for k in keys):
                raise InputError('Use 1 to 5 named keys.')
            item['keys']=keys
        elif kind=='scroll':
            item['amount']=integer(action.get('amount',1),-100,100,'Scroll amount')
        elif kind=='wait':
...
def execute(actions, context, write, *, permitted=False, dry_run=True, backend=None):
    ...
    if not permitted:
        raise InputError('Desktop actions are locked. Start with --allow-desktop and confirm this specific run.')
    if backend is None:
        try:
            import pyautogui as backend
        except (ImportError,KeyError,OSError) as exc:
            raise InputError('Desktop automation needs PyAutoGUI and an unlocked graphical user session.') from exc
    backend.FAILSAFE=True
    backend.PAUSE=0.15
    screen=backend.size()
    for action in actions:
        if action['type'] in {'move','click'} and (action['x']>=screen[0] or action['y']>=screen[1]):
            raise InputError('A mouse coordinate is outside the primary screen.')
        if action['type'] in {'press','hotkey'} and any(k not in backend.KEYBOARD_KEYS for k in action['keys']):
    ...
    def pause(seconds):
        ...
        while time.monotonic()<end:
            context.check()
            if time.monotonic()>deadline:raise InputError('Desktop actions reached the 120 second limit.')
    ...
    pause(3)
    for action in actions:
        context.check();backend.failSafeCheck()
        if time.monotonic()>deadline:raise InputError('Desktop actions reached the 120 second limit.')
        kind=action['type']
        if kind=='move':backend.moveTo(action['x'],action['y'],duration=.2)
        elif kind=='click':backend.click(action['x'],action['y'],clicks=action['clicks'],interval=.1,button=action['button'])
        elif kind=='write':
            for start in range(0,len(action['text']),32):
                context.check();backend.failSafeCheck()
                backend.write(action['text'][start:start+32],interval=.01)
        elif kind=='press':backend.press(action['keys'])
        elif kind=='hotkey':backend.hotkey(*action['keys'])
        elif kind=='scroll':backend.scroll(action['amount'])
        elif kind=='wait':pause(action['seconds'])
        elif kind=='screenshot':
            stream=io.BytesIO();backend.screenshot().save(stream,format='PNG');write(action['filename'],stream.getvalue())
