...
from .safety import InputError, MAX_FILE_BYTES, checked_path, clean_name, unique_write, within
from .storage import Store


class BaseApplication:
    def __init__(self, root: Path, data: Path, *, passphrase: str | None = None, allow_desktop: bool = False):
        ...
        if resume and self.store.encrypted:
            try:
                if resume['action'] not in {'trigger/start', 'watch/start', 'monitor/start'}:
    ...
    def info(self) -> dict:
        return {**json.loads((self.root / 'project.json').read_text(encoding='utf-8')),
                'example_path': str(self.root / 'examples'), 'data_path': str(self.data),
                'resume_error': self.resume_error, 'capabilities': {**capabilities(), **advanced_capabilities(), 'encrypted_database': self.store.encrypted, 'desktop_allowed': self.allow_desktop}}
    ...
    def artifact(self, path: Path) -> dict:
        ...
        return {'name': path.name, 'path': str(path.relative_to(self.exports)),
    ...
    def browse(self, value: str | None) -> dict:
        ...
        try:
            ...
            raise InputError('This folder cannot be listed with the current OS permissions.') from exc
        return {'path': str(path), 'parent': str(path.parent), 'entries': rows,
    ...
    def common_post(self, action: str, body: dict) -> dict:
        if action == 'upload':
            ...
            return {'path': str(destination), 'name': destination.name, 'size': len(raw)}
        if action == 'backup':
