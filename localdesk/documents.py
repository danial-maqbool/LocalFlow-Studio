...
def advanced_capabilities() -> dict:
    ...
    return {'ocr': bool(shutil.which('tesseract')), 'pdf_tables': available('pdfplumber'),
            'pdf_render': available('pypdfium2'), 'encryption': available('cryptography'),
            'semantic_lsa': available('sklearn'), 'neural_embeddings': available('sentence_transformers'),
            'desktop_package': available('pyautogui'), 'video_repair': bool(shutil.which('ffmpeg')),
...
def document_request(path: Path, operation: str = 'text', **options) -> dict:
    ...
    try:
        result = subprocess.run([sys.executable, '-m', 'localdesk.document_worker'],
                                input=json.dumps(request), text=True, encoding='utf-8',
                                capture_output=True, timeout=90, check=False,
                                cwd=Path(__file__).resolve().parents[1])
    except subprocess.TimeoutExpired as exc:
        raise InputError('Document processing reached its 90 second limit. Split the input into smaller files.') from exc
    if result.returncode or len(result.stdout) > 12_000_000:
        raise InputError('The document worker stopped. Check the file and installed local tools.')
    try:
