...
class SemanticModel:
    def __init__(self, texts: list[str], model_path: str = ''):
        ...
        if model_path:
            ...
            allowed={'sentence_transformers.models.Transformer','sentence_transformers.models.Pooling','sentence_transformers.models.Normalize'}
            if not isinstance(modules,list) or any(not isinstance(m,dict) or m.get('type') not in allowed for m in modules):
                raise InputError('Only Transformer, Pooling, and Normalize model modules are accepted.')
            if not any(directory.rglob('*.safetensors')):
                raise InputError('Use local safetensors model weights. Pickle model files are not loaded.')
            ...
            try:
                ...
                raise InputError('Install requirements-neural.txt before selecting a neural model.') from exc
            self.encoder=SentenceTransformer(str(directory),device='cpu',local_files_only=True,
                                            trust_remote_code=False,model_kwargs={'use_safetensors':True})
            self.vectors=self.encoder.encode(texts,normalize_embeddings=True,show_progress_bar=False,batch_size=16)
            ...
            try:
                ...
                raise InputError('The selected documents contain no searchable word tokens.') from exc
            dimension=min(96, max(1,len(texts)-1), max(1,matrix.shape[1]-1))
            if dimension>=2:
                self.reducer=TruncatedSVD(n_components=dimension,algorithm='randomized',n_iter=5,random_state=0)
    ...
    def scores(self, query: str) -> list[float]:
        ...
        return [round(max(-1.0,min(1.0,float(v))),6) for v in np.asarray(result).ravel()]
...
class SearchCache:
    ...
    def search(self, records: list[dict], query: str, *, model_path='', limit=100, minimum=0.05):
        if not records:
            return {'results':[],'engine':'Local semantic search','chunks':0}
        fingerprint=hashlib.sha256(model_path.encode())
        expanded=[]
        for record in records:
            fingerprint.update(json.dumps([record.get('id',record.get('source','')),record.get('name',''),record.get('text','')],ensure_ascii=False).encode('utf-8'))
            for offset,part in chunks(str(record.get('name',''))+'\n'+str(record.get('text',''))):
                expanded.append({'record':record,'text':part,'offset':offset})
                if len(expanded)>MAX_CHUNKS:
                    raise InputError('Semantic search reached 5,000 chunks. Select a smaller folder or use keyword search.')
        if not expanded:
            return {'results':[],'engine':'Local semantic search','chunks':0}
        ...
        with self.lock:
            ...
            for part,score in zip(expanded,scores):
                if score<minimum:
                    continue
                row=part['record'];ident=row.get('id',row.get('source',''))
                if ident not in best or score>best[ident]['semantic_score']:
                    best[ident]={**{k:v for k,v in row.items() if k!='text'},
                                 'semantic_score':score,'snippet':part['text'][:320],
                                 'text_length':len(str(row.get('text',''))),'chunk_offset':part['offset'],
                                 'match_reason':'Local model similarity. Check the source text.'}
            hits=sorted(best.values(),key=lambda r:(-r['semantic_score'],str(r.get('id',''))))[:limit]
            return {'results':hits,'engine':self.model.method,'dimensions':self.model.dimension,
                    'chunks':len(expanded),'suggestions':[],'limit':limit,'limited':len(best)>limit,
...
def extractive_summary(text: str, sentences: int = 3) -> str:
    ...
    parts=[p.strip() for p in re.split(r'(?<=[.!?])\s+|\n+',str(text)[:MAX_CHARS]) if len(p.strip())>15][:300]
    ...
    selected=sorted(sorted(range(len(parts)),key=lambda i:(-scores[i],i))[:sentences])
