...
class VaultTests(unittest.TestCase):
    ...
    def test_failed_write_restores_last_durable_state(self):
        ...
        before = self.path.read_bytes()
        with patch(
            "localdesk.vault._atomic_ciphertext", side_effect=OSError("simulated disk full")
        ):
    ...
    def test_failed_sql_script_restores_schema(self):
        ...
        with self.assertRaises(sqlite3.Error):
            with store.connection() as db:
                db.executescript("CREATE TABLE unexpected(a); INSERT INTO no_such_table VALUES(1);")
