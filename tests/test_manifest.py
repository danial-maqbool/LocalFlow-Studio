...
spec = importlib.util.spec_from_file_location("manifest_check", ROOT / "scripts/materialize.py")
...
class ManifestCheckoutTests(unittest.TestCase):
    def test_git_text_checkout_endings_have_the_same_source_hash(self):
        ...
        for name in (
            ...
            with self.subTest(name=name):
                self.assertEqual(materialize.digest(b"first\r\nsecond\r\n", name), expected)
    ...
    def test_binary_and_nul_containing_fixtures_keep_exact_bytes(self):
        for name, raw in (
            ...
            with self.subTest(name=name):
                self.assertEqual(materialize.digest(raw, name), hashlib.sha256(raw).hexdigest())
