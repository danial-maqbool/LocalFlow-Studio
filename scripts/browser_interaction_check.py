...
def main():
    ...
    if name == "ActivityGraph":
        ...
        for i in range(20):
            make_image(
                work / f"image-{i:02}.png", text=f"PUBLIC INVOICE {i+1000}\nTotal amount 4200"
            )
    before = {p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in work.iterdir()}
    ...
    try:
        ...
        while time.monotonic() < deadline:
            match = re.search(r"http://127\.0\.0\.1:\d+", (base / "backend.log").read_text())
            if match:
                break
            time.sleep(0.1)
        else:
        ...
        if name == "LocalFlow-Studio":
            ...
            body = {
                ...
                "workflow": {
                    ...
                    "edges": [{"from": "scan", "to": "ocr"}, {"from": "ocr", "to": "csv"}],
                },
            }
        ...
        try:
            ...
            process.wait()
        log.close()
    (base / "interaction-report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
