    ...
    with tempfile.TemporaryDirectory(prefix='localflow-worker-check-') as temp:
        ...
        def start():
            ...
            process = subprocess.Popen(
                [sys.executable, '-u', str(ROOT / 'run.py'), 'worker', '--port', '0',
                 '--data-dir', str(data), '--passphrase-env', 'LOCALFLOW_TEST_KEY'],
        ...
        try:
            start()
            state = api('trigger/start', {'id': 'invoice-register', 'workspace': str(inputs), 'mode': 'watch', 'seconds': 15})
