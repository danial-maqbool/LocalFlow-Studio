    ...
    with tempfile.TemporaryDirectory() as temp, sync_playwright() as pw:
        ...
        try:
            ...
            if args.bridge:
                ...
                def bridge(_source, resource, options):
                    if not resource.startswith("/api/"):
                        raise ValueError("Only local API paths are allowed in this test.")
                    request = urllib.request.Request(
                        url + resource,
                        data=options.get("body", "").encode() if options.get("body") else None,
            ...
            if name == "LocalFlow-Studio":
                ...
                settle()
                checked("Workflow preview completes", page.locator(".node-result").count() == 6)
                ...
                settle()
                checked(
                    "Demo index returns search results", page.locator(".search-hit").count() > 0
                )
                ...
                settle()
                checked("Keyword filter returns one file", page.locator(".search-hit").count() == 1)
                ...
                settle()
                checked(
                    "Semantic search returns local results", page.locator(".search-hit").count() > 0
                )
                ...
                page.locator("[data-inspection]").filter(has_text="missing-directory").click()
                settle()
                ...
                settle()
                checked("Manual note appears", page.locator("[data-event]").count() == 9)
                ...
                settle()
                checked("Four example files are scanned", page.locator("[data-scan]").count() == 4)
                checked(
                    "Clean preview removes the sample email",
                    "alex@example.test" not in page.locator("#clean-preview").inner_text(),
                )
                ...
                page.locator("[data-scan]").filter(has_text="image-with-comment").click()
                settle()
                ...
                if page.locator("#add-region").count():
                    ...
                    checked(
                        ...
                        "1 selected regions" in page.locator("#region-count").inner_text(),
                    )
