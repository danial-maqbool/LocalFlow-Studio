...
def safe_target(name: str) -> Path:
    ...
    if path.is_absolute() or '..' in path.parts or '\\' in name or name.startswith('.git/'):
...
def main() -> None:
    ...
    parser.add_argument('--refresh', action='store_true', help='Update hashes after a reviewed formatting-only pass.')
    ...
    if source:
        if source['repository'] != 'danial-maqbool/LocalFlow-Studio' or not re.fullmatch('[0-9a-f]{40}', source['commit']):
            raise ValueError('Shared source must be the approved repository and an immutable commit.')
