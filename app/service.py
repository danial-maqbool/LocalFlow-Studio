...
def starter_workflow() -> dict:
    return {'id': 'invoice-register', 'name': 'Invoice register', 'version': 1,
            'description': 'Read invoice text, extract an ID, create named copies, and write a CSV register.',
            'nodes': [
                {'id': 'input', 'type': 'scan', 'label': 'Read invoice folder', 'x': 36, 'y': 52,
                 'config': {'pattern': 'invoice-*.txt'}},
                {'id': 'read', 'type': 'read', 'label': 'Read invoice text', 'x': 292, 'y': 52, 'config': {}},
                {'id': 'id', 'type': 'extract', 'label': 'Extract invoice ID', 'x': 548, 'y': 52,
                 'config': {'field': 'invoice_id', 'pattern': r'Invoice:\s*(INV-\d+)'}},
                {'id': 'name', 'type': 'name', 'label': 'Set clean file names', 'x': 548, 'y': 225,
                 'config': {'template': '${invoice_id}${ext}'}},
                {'id': 'copy', 'type': 'copy', 'label': 'Create new copies', 'x': 292, 'y': 225, 'config': {}},
                {'id': 'table', 'type': 'csv', 'label': 'Write invoice register', 'x': 36, 'y': 225,
                 'config': {'filename': 'invoice-register.csv', 'fields': ['invoice_id', 'name', 'copied_as', 'sha256']}}
            ], 'edges': [{'from': a, 'to': b} for a, b in [('input', 'read'), ('read', 'id'),
                       ('id', 'name'), ('name', 'copy'), ('copy', 'table')]]}
...
class Application(BaseApplication):
    def setup(self):
        ...
        if not self.store.one('SELECT id FROM workflows LIMIT 1'):
            ...
            self.save({'id': 'folder-backup', 'name': 'Folder backup', 'nodes': [
                {'id': 'read', 'type': 'scan', 'label': 'Read workspace', 'x': 45, 'y': 85, 'config': {'pattern': '*'}},
                {'id': 'zip', 'type': 'archive', 'label': 'Create ZIP backup', 'x': 330, 'y': 85,
                 'config': {'filename': 'workspace-backup.zip'}}], 'edges': [{'from': 'read', 'to': 'zip'}]})
        self.desktop_tokens = {}
    ...
    def save(self, workflow: dict) -> dict:
        ...
        with self.store.connection() as db:
            db.execute('INSERT INTO workflows(id,name,body,revision,updated) VALUES(?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET '
                       'name=excluded.name,body=excluded.body,revision=excluded.revision,updated=excluded.updated',
                       (ident, name, raw, revision, utcnow()))
            db.execute('INSERT INTO workflow_versions(workflow_id,revision,body,created) VALUES(?,?,?,?)',
    ...
    def state(self):
        return {'jobs': self.jobs.recent(), 'workflows': self.store.rows('SELECT id,name,revision,updated FROM workflows ORDER BY name'),
                'trigger': dict(self.trigger), 'node_types': NODE_TYPES, 'desktop_allowed': self.allow_desktop,
                'stats': {'workflows': self.store.one('SELECT COUNT(*) AS n FROM workflows')['n'],
                          'runs': self.store.one('SELECT COUNT(*) AS n FROM jobs')['n'],
                          'finished': self.store.one("SELECT COUNT(*) AS n FROM jobs WHERE status='done'")['n']}}

    def get(self, action, query):
        if action == 'workflow':
            return self.load(query.get('id', 'invoice-register'))
        if action == 'versions':
            return {'versions': self.store.rows('SELECT revision,created FROM workflow_versions WHERE workflow_id=? '
                                               'ORDER BY revision DESC LIMIT 40', (query.get('id', ''),))}
        return super().get(action, query)

    def run_workflow(self, workflow: dict, workspace: Path, dry: bool = True, stop_after=None, desktop_token=""):
        validate(workflow)
        ...
        if any(n['type'] == 'desktop' for n in workflow['nodes']):
            ...
            if not dry:
                ...
                fingerprint = hashlib.sha256(json.dumps({'workflow': workflow, 'workspace': str(workspace)}, sort_keys=True).encode()).hexdigest()
                if not self.allow_desktop or not expected or expected['expires'] < time.monotonic() or expected['hash'] != fingerprint:
                    raise InputError('Desktop actions need fresh confirmation for this exact workflow and workspace.')
        ...
        def work(context):
            ...
            engine = Engine(workspace, output, context, self.data, desktop_permitted=desktop_permitted, desktop_deadline=desktop_expires)
            locked = False
            try:
                if desktop_permitted:
                    locked = self.desktop_run_lock.acquire(blocking=False)
                    if not locked:
                        raise InputError('Another desktop workflow is running. Do not run concurrent mouse or keyboard workflows.')
                result = engine.run(snapshot, stop_after=stop_after)
            finally:
                if locked:
                    self.desktop_run_lock.release()
            result['workflow'] = {'id': snapshot.get('id'), 'name': snapshot.get('name'),
            ...
            if output:
                ...
                unique_write(output / 'run-manifest.json', json.dumps(
                    {'workflow': export_snapshot, 'run': result}, ensure_ascii=False, indent=2).encode('utf-8'))
    ...
    def start_trigger(self, body):
        ...
        self.trigger_stop = threading.Event()
        self.trigger = {'enabled': True, 'mode': mode, 'seconds': seconds,
                        'workspace': str(workspace), 'workflow': workflow['name'],
                        'last_error': '', 'last_job': None}
        def snapshot():
            return tuple((str(p), p.stat().st_mtime_ns, p.stat().st_size)
                         for p in walk_files(workspace, limit=5000) if not within(p, self.data))
        current_signature = snapshot()
        identity = hashlib.sha256(json.dumps({'workspace': str(workspace), 'workflow': workflow}, sort_keys=True).encode()).hexdigest()
        saved = self.store.get('trigger_signature', {})
        baseline = tuple(tuple(row) for row in saved.get('files', [])) if self.resuming_background and saved.get('identity') == identity else current_signature
    ...
    def post(self, action, body):
        if action == 'desktop/arm':
            if not self.allow_desktop or body.get('confirmed') is not True:
                raise InputError('Enable --allow-desktop and review the desktop actions before confirming a run.')
            workflow = body.get('workflow', {})
            validate(workflow)
            workspace = checked_path(body.get('workspace', ''), directory=True)
            fingerprint = hashlib.sha256(json.dumps({'workflow': workflow, 'workspace': str(workspace)}, sort_keys=True).encode()).hexdigest()
            self.desktop_tokens = {k: v for k, v in self.desktop_tokens.items() if v['expires'] >= time.monotonic()}
        ...
        if action == 'run':
            ...
            return {'job_id': self.run_workflow(workflow, checked_path(body.get('workspace', ''), directory=True),
                                                 bool(body.get('dry_run', True)), body.get('stop_after'), str(body.get('desktop_token', '')))}
        if action == 'export':
            workflow = self.load(str(body.get('id', 'invoice-register')))
        ...
        if action == 'restore':
            row = self.store.one('SELECT body FROM workflow_versions WHERE workflow_id=? AND revision=?',
