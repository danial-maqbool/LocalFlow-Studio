...
def convert(workflow):
    ...
    for index, value in enumerate(steps):
        ...
        node=json.loads(json.dumps(value));node.setdefault('id',str(index));node.setdefault('config',{})
        if node.get('type')=='rename':node['type']='name'
        ...
        nodes.append(node)
    return {**{k:v for k,v in workflow.items() if k!='steps'},'nodes':nodes,
...
def upgrade_schema(store):
    ...
    with store.connection() as db:
        ...
        for row in list(db.execute('SELECT id,name,body FROM workflows')):
            workflow=convert(json.loads(row['body']));workflow.update({'id':row['id'],'name':row['name'],'version':1})
            validate(workflow)
            db.execute('UPDATE workflows SET body=?,updated=? WHERE id=?',(json.dumps(workflow),utcnow(),row['id']))
