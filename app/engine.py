...
NODE_TYPES = {
    "ocr": {
        "label": "Read scanned document",
        "detail": "Read images and scanned PDFs with local Tesseract OCR.",
    },
    "tables": {
        "label": "Extract PDF tables",
        "detail": "Create records from PDF table rows. Choose lines, text, or OCR.",
    },
    "semantic": {
        "label": "Semantic filter",
        "detail": "Rank records with a locally trained model or local neural weights.",
    },
    "summary": {
        "label": "Summarize text",
        "detail": "Select source sentences with a local statistical model.",
    },
    "desktop": {
        "label": "Desktop actions",
        "detail": "Run explicitly confirmed mouse and keyboard actions. Never in a background trigger.",
    },
    ...
    "extract": {
        "label": "Extract field",
        "detail": "Extract one field with a bounded regular expression.",
    },
    ...
    "merge": {
        ...
        "detail": "Merge parent records and remove repeated sources.",
    },
...
def validate(workflow: dict) -> tuple[list[dict], dict[str, list[str]]]:
    ...
    for node in nodes:
        if not isinstance(node, dict) or not re.fullmatch(
            r"[A-Za-z0-9_-]{1,48}", str(node.get("id", ""))
        ):
...
def regex_field(pattern: str, text: str) -> str:
    ...
    try:
        ...
        raise InputError(
            "The extraction pattern exceeded its time limit. Use a simpler pattern."
        ) from exc
    ...
    def run(self, workflow: dict, *, stop_after: str | None = None) -> dict:
        ...
        for index, node in enumerate(nodes):
            ...
            self.context.progress(
                int(100 * index / len(nodes)), f"Running {node.get('label', node['type'])}"
            )
            ...
            steps.append(
                {
                    ...
                    "records": len(data),
                    "seconds": round(time.monotonic() - mark, 4),
                    "note": note,
                    "preview": [{k: v for k, v in row.items() if k != "text"} for row in data[:5]],
                }
            )
        ...
        return {
            "dry_run": self.output is None,
            "steps": steps,
            "record_count": len(last),
            "planned_files": self.planned,
            "seconds": round(time.monotonic() - started, 4),
            "stopped_after": stop_after,
            "source_files_changed": None if self.desktop_used else False,
    ...
    def step(self, node: dict, rows: list[dict]) -> tuple[list[dict], str]:
        ...
        if kind == "filter":
            ...
            return [
                ...
                and (not extension or r["ext"].lower() == extension)
            ], "Files filtered."
        if kind == "ocr":
            ...
            for row in rows:
                self.context.check()
                parsed = document_request(
                    self.source(row), "text", ocr=True, language=str(config.get("language", "eng"))
                )
            ...
            return (
                rows,
                "Local OCR complete. Review recognition errors before using extracted values.",
            )
        if kind == "tables":
            from localdesk.documents import document_request
            ...
            for row in rows:
                ...
                parsed = document_request(
                    self.source(row),
                    "tables",
                    strategy=config.get("strategy", "lines"),
                    language=config.get("language", "eng"),
                    column_edges=config.get("column_edges"),
                )
                self.source(row)
                for table in parsed["tables"]:
                    for index, values in enumerate(table["rows"]):
                        ...
                        if len(output) > 500:
                            raise InputError(
                                "This workflow has more than 500 table rows. Split the PDF input."
                            )
        ...
        if kind == "semantic":
            ...
            model = SemanticModel(
                [str(r.get("text", r["name"])) for r in rows], str(config.get("model_path", ""))
            )
        ...
        if kind == "summary":
            ...
            return (
                rows,
                "Selected existing source sentences. No generated claims or external model calls.",
            )
        if kind == "desktop":
            if self.output is not None and (
                self.desktop_deadline is None or time.monotonic() > self.desktop_deadline
            ):
                raise InputError(
                    "Desktop confirmation expired before this step started. Preview first, then confirm a fresh run."
                )
            ...
            note = execute(
                ...
                self.context,
                self.write,
                permitted=self.desktop_permitted,
                dry_run=self.output is None,
            )
        ...
        if kind == "extract":
            ...
            if not re.fullmatch("[A-Za-z][A-Za-z0-9_]{0,30}", field) or field in {
                "source",
                "sha256",
                "name",
                "stem",
        ...
        if kind == "condition":
            field, operator, value = (
                str(config.get("field", "value")),
                config.get("operator", "not_empty"),
            ...
            ], f"Kept records that pass {field}: {operator}."
        if kind == "name":
            for row in rows:
        ...
        if kind == "copy":
            for row in rows:
                ...
                row["copied_as"] = name
            return rows, (
                f"{len(rows)} copies planned."
                if not self.output
                else f"{len(rows)} new copies written."
            )
        if kind == "csv":
            ...
            name = self.write(
                str(config.get("filename", "records.csv")), text.getvalue().encode("utf-8-sig")
            )
            return rows, f"Table: {name}"
        if kind == "json":
            public = [{k: v for k, v in r.items() if k != "text"} for r in rows]
            name = self.write(
                str(config.get("filename", "records.json")),
