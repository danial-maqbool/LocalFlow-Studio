...
def main() -> int:
    ...
    parser.add_argument(
        "--demo", action="store_true", help="Use temporary example data and an in-memory database."
    )
    parser.add_argument(
        "--keyring", action="store_true", help="Unlock with the explicitly saved OS credential."
    )
    parser.add_argument(
        "--passphrase-env",
        metavar="VARIABLE",
    ...
    try:
        ...
        if not args.demo:
            if args.keyring and not (args.command == "service" and args.install):
                ...
                if not password:
                    raise InputError("The named password environment variable is empty.")
            else:
